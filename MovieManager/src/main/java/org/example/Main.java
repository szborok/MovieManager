...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Room number: " +
                Logic.leastScreeningRoom(cinemacityCsepel));
        System.out.println("---");

        System.out.println("Room(s) with more than 3 movies. (Random results)");
        for (int i = 0; i < Logic.roomsWithMoreThanThreeMovies(cinemacityCsepel).size(); i++) {
            System.out.println("Room number: " +
                    Logic.roomsWithMoreThanThreeMovies(cinemacityCsepel).get(i));
        }
        System.out.println("---");

        System.out.println("Closed rooms with their capacity. (Constant results)");
        Logic.closedRooms(cinemacityCsepel);
        System.out.println("---");

        Logic.roomDetails(cinemacityCsepel);
